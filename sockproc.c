...
int get_proc_from_conn(struct phx_conn_data* c,int direction)
{
  ...
  const char* f6name = "/proc/net/tcp6";
    ...
    parse_tcp_line(buf,s,d,&sport,&dport,&socknum);
		...
		{
			if ( (dport == c->dport && sport == c->sport && !strncmp(s,(gchar*)c->srcip,4) && !strncmp(d,(gchar*)c->destip,4) ) ||
				(dport == c->sport && sport == c->dport && !strncmp(s,(gchar*)c->destip,4) && !strncmp(d,(gchar*)c->srcip,4) ) )
			{
				char fname[100];
				char procname[1024];
				int pnlen;
