...
{
    log_debug ("GUI, Signal caught, exiting...\n");
    gtk_main_quit ();
}
...
gboolean
gui_timer_callback (gpointer data)
{
	...
	dialog =
	    gtk_message_dialog_new ((GtkWindow *) NULL,
				    GTK_DIALOG_DESTROY_WITH_PARENT,
				    GTK_MESSAGE_WARNING, GTK_BUTTONS_YES_NO,
				    "A program %s wants to reach internet\n %s:%d -> %s:%d",
				    conndata->proc_name->str, srcip->str,
				    conndata->sport, destip->str,
				    conndata->dport);
    ...
    log_debug("GUI Running messag dialog...\n");
    gint resp = gtk_dialog_run (GTK_DIALOG (dialog));
    
    gtk_widget_destroy ((GtkWidget *) dialog);
    log_debug("GUI Message dialog has benn answered!\n");
...
main (int argc, char **argv)
{
    ...
    setup_socket ();
    htimeout =
	g_timeout_add ((guint32) 10, gui_timer_callback, my_callback_data);
    gtk_main ();
