...
gpointer
daemon_socket_thread (gpointer data)
{
    int socklen = 0;
...
send_conn_data (struct phx_conn_data *data)
{
...
process_gui_queue ()
{
	...
	{
	    log_debug ("Found NULL process name, something went wrong\n");
	    return;
	}
	rule =
...
timer_callback (gpointer data)
{
	...
	{
	    while ((rv = recv (out_fd, buf, sizeof (buf), MSG_DONTWAIT))
		   && rv > 0)
	    {
	...
	{
	    while ((rv = recv (in_fd, buf, sizeof (buf), MSG_DONTWAIT))
		   && rv > 0)
	    {
...
*/
gpointer
pending_thread_run (gpointer data)
{
	...
	if (ret > 0)
	{
		...
		{
		    rv = recv (out_pending_fd, buf, sizeof (buf),
			       MSG_DONTWAIT);
		    if (rv > 0)
		    {
			log_debug
		...
		{
		    rv = recv (in_pending_fd, buf, sizeof (buf),
			       MSG_DONTWAIT);
		    if (rv > 0)
		    {
			log_debug ("Packet received in inbound pending queue\n");
...
void
close_queue (struct nfq_handle *handle, struct nfq_q_handle *qhandle)
{
